...
mod tests {
    ...
    fn session_removed_emits_dashboard_updated() {
        ...
        let events = drain_events(&mut rx);
        // Count changed → dashboard updated; no SessionUpdated for removed sessions
        assert!(
            events
                .iter()
                .any(|e| matches!(e, AppEventKind::DashboardUpdated)),
    ...
    fn new_run_emits_run_updated_and_dashboard_updated() {
        ...
        let events = drain_events(&mut rx);
        assert!(
            events.iter().any(|e| matches!(e, AppEventKind::RunUpdated { team_run_id, .. } if team_run_id == "run-abc")),
    ...
    fn run_status_change_emits_run_updated() {
        ...
        assert!(
            events.iter().any(
                |e| matches!(e, AppEventKind::RunUpdated { status, .. } if status == "completed")
            ),
    ...
    fn multiple_session_changes_emit_multiple_session_updated_events() {
        ...
        current
            .sessions
            .insert("session-a".to_string(), "t2".to_string()); // updated
        current
            .sessions
            .insert("session-b".to_string(), "t1".to_string()); // unchanged
        current
            .sessions
            .insert("session-c".to_string(), "t1".to_string()); // new
        ...
        let session_events: Vec<_> = events
            ...
            .collect();
        // session-a updated + session-c new = 2 SessionUpdated events
        assert_eq!(
            session_events.len(),
            2,
