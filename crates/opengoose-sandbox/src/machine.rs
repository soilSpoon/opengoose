...
pub fn create_dtb_with_initrd(ram_size: u64, initrd: Option<&InitrdInfo>) -> Result<Vec<u8>> {
    ...
    {
        let mem = fdt
            .begin_node(&format!("memory@{RAM_BASE:x}"))
            .map_err(map_err)?;
        fdt.property_string("device_type", "memory")
            .map_err(map_err)?;
        fdt.property("reg", &prop64(&[RAM_BASE, ram_size]))
            .map_err(map_err)?;
    ...
    // GICv3
    {
        ...
        fdt.property(
            "interrupts",
            &prop32(&[GIC_FDT_IRQ_TYPE_PPI, 9, IRQ_TYPE_LEVEL_HI]),
        )
        .map_err(map_err)?;
    ...
    {
        let uart_node = fdt
            .begin_node(&format!("uart@{UART_ADDR:x}"))
            .map_err(map_err)?;
        ...
        fdt.property("reg", &prop64(&[UART_ADDR, UART_SIZE]))
            .map_err(map_err)?;
        fdt.property(
            "interrupts",
            &prop32(&[GIC_FDT_IRQ_TYPE_SPI, uart::PL011_IRQ, IRQ_TYPE_LEVEL_HI]),
        )
        .map_err(map_err)?;
    ...
    {
        let virtio = fdt
            .begin_node(&format!("virtio_mmio@{VIRTIO_MMIO_BASE:x}"))
            .map_err(map_err)?;
        fdt.property_string("compatible", "virtio,mmio")
            .map_err(map_err)?;
        fdt.property("reg", &prop64(&[VIRTIO_MMIO_BASE, VIRTIO_MMIO_SIZE]))
            .map_err(map_err)?;
        fdt.property(
            "interrupts",
            &prop32(&[GIC_FDT_IRQ_TYPE_SPI, VIRTIO_IRQ, IRQ_TYPE_LEVEL_HI]),
        )
        .map_err(map_err)?;
    ...
    // Chosen
    {
        ...
        fdt.property_string(
            "bootargs",
            "console=ttyAMA0 earlycon=pl011,0x09000000 reboot=t panic=-1",
        )
        .map_err(map_err)?;
        fdt.property_string("stdout-path", &format!("/uart@{UART_ADDR:x}"))
            .map_err(map_err)?;
        if let Some(initrd) = initrd {
            fdt.property_u64("linux,initrd-start", initrd.start_gpa)
                .map_err(map_err)?;
            fdt.property_u64("linux,initrd-end", initrd.end_gpa)
                .map_err(map_err)?;
        }
