...
impl Engine {
    ...
    pub fn handle_team_command(&self, session_key: &SessionKey, args: &str) -> String {
        match args {
            "" => match self.session_manager.active_team_for(session_key) {
            ...
            "list" => {
                let teams = self.session_manager.list_teams();
                if teams.is_empty() {
            ...
            team_name => {
                if self.session_manager.team_exists(team_name) {
                    self.session_manager
                        .set_active_team(session_key, team_name.to_string());
                    format!("Team {team_name} activated for this channel.")
                } else {
                    let available = self.session_manager.list_teams();
                    format!(
