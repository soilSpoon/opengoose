...
mod tests {
    ...
    #[test]
    fn parse_web_subcommand_with_host_and_port() {
        let cli = Cli::parse_from(["opengoose", "web", "--host", "0.0.0.0", "--port", "9090"]);
    ...
    #[test]
    fn parse_alert_test_with_rule_and_dry_run() {
        let cli = Cli::parse_from([
            "opengoose",
            "alert",
            "test",
        ...
        match cli.command {
            Some(Command::Alert {
                action: cmd::alert::AlertAction::Test { rule, dry_run },
            }) => {
                assert_eq!(rule, Some("queue-backlog".into()));
                assert!(dry_run);
            }
            _ => panic!("expected Alert test command"),
        }
    }

    #[test]
    fn parse_alert_history_with_filters() {
        let cli = Cli::parse_from([
            "opengoose",
            "alert",
            "history",
        ...
        match cli.command {
            Some(Command::Alert {
                action: cmd::alert::AlertAction::History {
                    limit,
                    rule,
                    since,
                },
                ...
                assert_eq!(limit, 25);
                assert_eq!(rule, Some("queue-backlog".into()));
                assert_eq!(since, Some("24h".into()));
            }
            _ => panic!("expected Alert history command"),
        }
    }
