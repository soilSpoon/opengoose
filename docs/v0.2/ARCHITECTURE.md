...
│   │
│   └── opengoose-rig/            # [스텁] Agent Rig (영속 pull 루프)
│       └── src/
│           ├── lib.rs
...
impl Rig {
    ...
    async fn execute(&mut self, work: WorkItem) {
        ...
        let prime = self.board.prime(&self.id);  // Layer 1 + Layer 2 TREE.md
        ...
        //    에이전트는 실행 중 board__remember/board__recall 자유 사용
        ...
        // 7. 결과 스트리밍 + pre-compaction flush 모니터링
        self.process_stream_with_flush(stream, &work).await;
