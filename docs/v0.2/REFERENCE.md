...
- 프록시 데몬이 포트 1355에서 서브도메인을 랜덤 백엔드 포트로 라우팅
...
## 7. Open SWE (langchain-ai/open-swe)

**무엇인가:** Stripe Minions, Ramp Inspect, Coinbase Cloudbot의 프로덕션 패턴을 재사용 가능한 코딩 에이전트 아키텍처로 체계화한 오픈소스 프레임워크.

**채택한 핵심 컨셉:**

### 미들웨어 훅 (Deep Agents 패턴)
...
## 8. Stripe Minions (stripe.dev)

**무엇인가:** Stripe의 내부 코딩 에이전트 시스템. 주당 1,300+ PR 머지. Goose 포크 위에 구축.

**핵심 통찰: "모델이 시스템을 운영하는 게 아니다. 시스템이 모델을 운영한다."**

채택한 패턴:
- **블루프린트 패턴** — 결정론적 + 에이전트 노드 교차
- **컨텍스트 사전 수집** — 에이전트 루프 전 모든 참조를 결정론적으로 fetch
- **제한된 반복** — CI 수정 최대 2라운드
- **범위 지정 규칙 파일** — 디렉토리별 코딩 규칙 (Cursor 포맷), 에이전트 간 공유
- **Devbox 격리** — 사전 워밍된 인스턴스, 10초 스핀업, 네트워크 격리

---

## 9. Ramp Inspect (modal.com)

**무엇인가:** Ramp의 백그라운드 코딩 에이전트. 전체 머지 PR의 ~30%. OpenCode + Modal 샌드박스 위에 구축.

**핵심 통찰: 전체 컨텍스트 샌드박스 = 에이전트가 인간 엔지니어와 같은 도구를 가짐.**

채택한 패턴:
- **샌드박스 안에 전체 개발 환경** — Postgres, Redis, Temporal, 모든 서비스 실행
- **스냅샷 기반 시작** — 30분마다 파일시스템 스냅샷 재구축, 거의 즉시 시작
- **워밍 프리로딩** — 사용자가 타이핑 시작할 때 샌드박스가 스핀업 시작
- **전후 스크린샷 비교** — 프론트엔드 에이전트가 시각적 변경사항 검증

---

## 10. Coinbase Cloudbot (coinbase.com)

**무엇인가:** Coinbase의 엔터프라이즈 코딩 에이전트 프레임워크.

**핵심 통찰: 관찰 가능성과 감사 가능성은 부수적 관심사가 아닌 경성 요구사항.**

채택한 패턴:
- **코드 우선 그래프 아키텍처** — 타입된 인터페이스, 버전 관리, 깔끔한 노드 분리
- **결정론적 + 확률적 분리** — 데이터 노드는 유닛 테스트, LLM 노드는 eval 하니스 사용
- **모든 것을 추적** — 모든 도구 호출, 검색, 결정, 출력을 추적하고 diff 가능
...
Beads (ready/prime/compact, wisps, 해시 ID, landing)
...
Portless (네임드 URL, worktree 감지)
  → 우리의 rig 격리에 영향 (포트 충돌 없음)

Gas Town (witness, GUPP, re-imagine 머지)
  → 우리의 운영 패턴에 영향

Open SWE / Deep Agents (미들웨어, 컨텍스트 사전 수집, 안전망)
  → 우리의 rig 미들웨어 파이프라인에 영향

Stripe Minions (블루프린트 패턴, 제한된 반복, 큐레이션된 도구)
  → 우리의 결정론적 + 에이전트 노드 실행 모델에 영향

Ramp Inspect (전체 컨텍스트 샌드박스, 스냅샷 시작, 워밍 풀)
  → 우리의 향후 샌드박스 추상화에 영향
```
