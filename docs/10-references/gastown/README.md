...
> **Note:** Gastown은 Dolt를 사용하는 원본 시스템이다. **OpenGoose v2는 Dolt를 사용하지 않으며, 관련 패턴을 prollytree 위에 재구현한다.**

Gastown is a multi-agent orchestration paradigm (75k LOC Go) designed to manage dozens of agents in parallel, eliminating the "baby-sitting" overhead often associated with AI agents.
...
- **Mail System**: 4 priority × 4 type 구조화된 메일 큐 (Queue/Interrupt 전달 모드)
- **GUPP (Propulsion Principle)**: "Hook에 있으면 실행한다" — Pull 기반 실행 모델
- **Convoy**: 다중 이슈를 묶어서 추적, 완료 시 자동 알림

## OpenGoose 적용

| Gastown | OpenGoose v2 |
|---------|--------------|
| Dolt | **prollytree** (순수 Rust, 3-way merge) |
| Mayor | User (채널 메시지 전송자) |
| Witness | witness.rs (EventBus + 타이머) |
| Polecat | FanOut/Chain executor의 에이전트 |
| Refinery | "re-imagine" Recipe + prollytree merge |
| Mail | AgentMessageStore + MCP team tools |
| Convoy | orchestration_runs 확장 |
